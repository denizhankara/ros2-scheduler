...
class PriorityNode(Node):
    ...
    def create_publisher_priority(self, levels, msg_type, topic: str, qos_profile: QoSProfile | int, *, callback_group: CallbackGroup | None = None, event_callbacks: PublisherEventCallbacks | None = None, qos_overriding_options: QoSOverridingOptions | None = None, publisher_class: Type[Publisher] = ...) -> Publisher:
        for i in range(len(levels)):
            publisher = self.create_publisher(msg_type, topic+'_'+str(i), qos_profile, callback_group=callback_group,
                                              event_callbacks=event_callbacks, qos_overriding_options=qos_overriding_options, publisher_class=publisher_class)
    ...
    def create_subscription_priority(self, levels, msg_type, topic: str, callback: Callable[[MsgType], None], qos_profile: QoSProfile | int, *, callback_group: CallbackGroup | None = None, event_callbacks: SubscriptionEventCallbacks | None = None, qos_overriding_options: QoSOverridingOptions | None = None, raw: bool = False) -> Subscription:
        for i in range(len(levels)):
